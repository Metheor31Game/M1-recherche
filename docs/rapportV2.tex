...
\textbf{Format d'entrée}
    ...
    \end{itemize}
    On a donc 24 jeux de données différents, structurés en deux grandes campagnes de tests permettant d'isoler l'influence de la diversité des prédicats :

    \begin{itemize}
        \item \textbf{Jeux 1 à 12 (Signature restreinte) :} Utilisation de 3 symboles de prédicats $\{P, Q, R\}$.
        \item \textbf{Jeux 13 à 24 (Signature étendue) :} Utilisation de 6 symboles de prédicats $\{P, Q, R, S, T, U\}$.
    \end{itemize}

    \newpage

    Pour chacune de ces deux campagnes, nous avons croisé quatre niveaux de profondeur avec trois échelles de taille, comme l'illustre la répartition des jeux 13 à 24 ci-dessous :

    \begin{table}[H]
    \centering
    \begin{tabular}{|l|c|c|c|}
    \hline
    \textbf{Profondeur / Taille} & \textbf{10 000} & \textbf{100 000} & \textbf{1 000 000} \\ \hline
    \textbf{Profondeur 1}        & Jeu 13          & Jeu 14           & Jeu 15             \\ \hline
    \textbf{Profondeur 5}        & Jeu 16          & Jeu 17           & Jeu 18             \\ \hline
    \textbf{Profondeur 10}       & Jeu 19          & Jeu 20           & Jeu 21             \\ \hline
    \textbf{Profondeur 20}       & Jeu 22          & Jeu 23           & Jeu 24             \\ \hline
    \end{tabular}
    \caption{Répartition des jeux de données pour la signature étendue}
    \end{table}

    Cette organisation nous permet d'analyser précisément comment les algorithmes réagissent lorsque l'on fait varier un seul facteur (par exemple la profondeur) tout en gardant les autres (taille et signature) constants.
    
\vspace{0.2cm}
\textbf{Protocole de mesure et format de sortie}

    Chacun de nos test sont exécutés dix fois. Pour chaque série, nous retirons le pire temps et le meilleur afin d'éliminer d'éventuels biais ponctuels, puis nous calculons la moyenne des résultats restants.

    \begin{itemize}
        \item L'export des données s'effectue sous un format facilement exploitable (CSV) ;
        \item Le fichier de sortie contient les données suivantes : jeu de données, algorithme, structure de données utilisée, mode d'unification (exhaustive ou première occurrence), temps de prétraitement, temps total, consommation de RAM et charge CPU.
    \end{itemize}

    \begin{remarque}
        Un fichier CSV intermédiaire est également généré. Il contient l'intégralité des itérations ainsi que le nombre total d'unifications réalisées, ce qui permet de vérifier la cohérence des résultats.
    \end{remarque}
