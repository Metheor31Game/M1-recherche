...
des objets manipulés doit non seulement être correcte, mais aussi permettre 
un traitement automatique efficace. C'est précisément ce défi qui est au cœur 
de ce rapport.
...
\begin{itemize}
    ...
    \item Ensemble de symboles de \textit{prédicats}~: $P, Q, R$, etc. Ils expriment des propriétés ou des relations entre objets, e.g.~$\mathit{Homme}(\mathit{socrate})$ exprime que Socrate est un homme.
\end{itemize}
...
\begin{paraph}{Détails d'implémentation et spécificités Python}
    ...
    certaines «~pythonneries\textregistered\footnote{Terme déposé par l'équipe 
