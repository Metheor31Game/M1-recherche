...
\begin{paraph}{Avantages}
    \begin{itemize}
        ...
        prédicat dans la base d'unification se fait en $\mathcal{O}(1)$. 
...
\begin{paraph}{Inconvénients}
    ...
    un à un à tous les éléments de la liste ($\mathcal{O}(N)$  tentatives). 
...
\subsubsection{Martelli-Montanari}
...
\begin{paraph}{Insertion}
    ...
    est déjà partagé à une autre branche).

    \newpage
    Soit $n$ la taille d'un prédicat, i.e~son nombre total de symboles.
    Chaque symbole nécessite~:
    \begin{itemize}
        \item Une consultation des fils du nœud courant~;
        \item La création éventuelle d'un nouveau nœud~;
        \item Une descente dans l'arbre.
    \end{itemize}

    La complexité temporelle de l'insertion est donc~:
    \[
    \mathcal{O}(n)
    \]
\end{paraph}

\begin{paraph}{Filtrage structurelle}
